...
if ($action == 'cannotsendmessage') {
    $msg = $OUTPUT->heading('The bulk email capability has been disabled for teachers in this test course');
    $url = new moodle_url('/mod/questionnaire/show_nonrespondents.php', array('id'=>$cm->id));
    redirect($url, $msg, 4);
    exit;
}
...
if (!$students) {
    ...
    if ($showall) {
        ...
        echo '<div>';
        echo '<label style="float:left;" for="questionnaire_subject">'.get_string('subject', 'questionnaire').'&nbsp;</label>';
        echo '<input style="float:left;" type="text" id="questionnaire_subject" size="70" maxlength="255" name="subject" value="'.$subject.'" />';
        echo '</div>';
        echo '<div class="clearer"></div>';
