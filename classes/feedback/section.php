...
class section {
     ...
     *  'surveyid' - the survey_id field of the fb_sections table (required if no 'id' field),
    ...
    public static function new_section($surveyid, $sectionlabel = '') {
        ...
        $maxsection = $DB->get_field(self::TABLE, 'MAX(section)', ['survey_id' => $surveyid]);
        $newsection->survey_id = $surveyid;
    ...
    public function load_section($params) {
        ...
        if (!is_array($params)) {
            ...
            $where = 'WHERE fs.survey_id = :surveyid AND fs.section = :sectionnum ';
        ...
        $from = 'FROM {' . self::TABLE . '} fs LEFT JOIN {' . sectionfeedback::TABLE . '} f ON fs.id = f.section_id ';
        ...
        foreach ($feedbackrecs as $fbid => $feedbackrec) {
            if (empty($this->id)) {
                ...
                $this->survey_id = $feedbackrec->survey_id;
    ...
    public function delete() {
        ...
        $allsections = $DB->get_records(self::TABLE, ['survey_id' => $this->survey_id], 'section ASC');
        $count = 1;
    ...
    public function delete_sectionfeedback() {
        ...
        $DB->delete_records(sectionfeedback::TABLE, ['section_id' => $this->id]);
