            ...
            foreach ($responsedata->{$qname} as $choiceid => $choicevalue) {
                if ($choicevalue) {
                    ...
                    if (isset($question->choices[$choiceid]) && $question->choices[$choiceid]->is_other_choice()) {
                        $cname = \mod_questionnaire\local\question\choice::id_other_choice_name($choiceid);
                        $record->value =
        ...
        if (!empty($records)) {
            ...
            foreach ($records as $row) {
                ...
                if (\mod_questionnaire\local\question\choice::content_is_other_choice($row->ccontent)) {
                    $newrow['responses'][\mod_questionnaire\local\question\choice::id_other_choice_name($row->cid)] =
                        $row->response;
                }
            }
            $values[$qid] = $newrow;
