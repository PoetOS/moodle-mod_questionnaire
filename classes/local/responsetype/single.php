...
class single extends responsetype {
    ...
    public static function answers_from_appdata($responsedata, $question) {
        ...
        if (isset($responsedata->{$qname}[0]) && !empty($responsedata->{$qname}[0])) {
            ...
            if ($question->choices[$record->choiceid]->is_other_choice()) {
                $cname = \mod_questionnaire\local\question\choice::id_other_choice_name($record->choiceid);
                $record->value =
        ...
        if ($rows) {
            ...
            foreach ($rows as $idx => $row) {
                if (strpos($idx, 'other') === 0) {
                    ...
                    $content = \mod_questionnaire\local\question\choice::content_other_choice_display($ccontent);
    ...
    public static function response_select($rid) {
        ...
        foreach ($records as $row) {
            ...
            if (\mod_questionnaire\local\question\choice::content_is_other_choice($row->ccontent)) {
                $newrow['responses'][\mod_questionnaire\local\question\choice::id_other_choice_name($row->cid)] = $row->response;
            }
