...
/**
 * Builds the RGraph JS chart markup consumed by feedback::build_scoreboard().
 *
 * Pure-ish renderer: given the chart inputs, returns a string of HTML + inline
 * <script> that initialises the RGraph visualisation. Pure function: input
 * args in, HTML+JS string out. No state, no globals, no side effects.
 */
class chart_renderer {
     ...
     * @param array $labels Chart axis labels.
     ...
     * @param array|null $allscore The "group" series (used for compare / allresponses paths).
     * @param string|null $globallabel Label used in 'global' feedbacktype mode.
     * @param string $charttitle First-chart title text (e.g. "Your response" or "This response").
     * @return string Rendered HTML + JS for the chart.
     */
    public static function render(
        ...
        // Make gradient colors customizable in the settings (future enhancement).
        ...
        switch ($charttype) {
            case 'bipolar':
                ...
                if (!$allresponses) {
                    $pageoutput .= '
                            <canvas id="cvs" width="' . $canvaswidth . '" height="' . $canvasheight .
                                '">[No canvas support]</canvas>
                        ';
                }
                if ($allscore) {
                    $pageoutput .= '
                            <canvas id="cvs2" width="' . $canvaswidth . '" height="' . $canvasheight .
                                '">[No canvas support]</canvas>
                        ';
                }
                $pageoutput .= '
