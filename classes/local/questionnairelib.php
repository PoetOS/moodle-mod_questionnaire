...
class questionnairelib {
    ...
    public function survey_print_render($courseid, $message = '', $referer = '', $rid = 0, $blankquestionnaire = false) {
        ...
        if (data_submitted()) {
            ...
            foreach ($this->questionsbysec as $section) {
                ...
                if ($errormessage) {
                    if ($numsections > 1) {
                        $pageerror = get_string('page', 'questionnaire') . ' questionnaire.php' . $s . ' : ';
                    }
                    $this->page->add_to_page(
                        'notifications',
        ...
        for ($i = 0; $i < count($answers[0]); $i++) {
            ...
            switch ($i) {
                case 1:
                    ...
                    break;
                case 4:
                    $formatted['plaintext'] .= get_string('user') . ' questionnaire.php';
                    $formatted['html'] .= get_string('user') . ' questionnaire.php';
                    break;
                case 6:
                    if ($this->respondenttype != 'anonymous') {
    ...
    private function response_send_email($rid, $email) {
        ...
        $bodyhtml .= get_string('surveyresponse', 'questionnaire') . ' questionnaire.php' . $name . '"' . $endhtml;
        $bodyplaintext .= get_string('surveyresponse', 'questionnaire') . ' questionnaire.php' . $name . '"' . $endplaintext;
    ...
    public function survey_results($rid = '', $uid = false, $pdf = false, $currentgroupid = '', $sort = '') {
        ...
        if (empty($this->questions)) {
            $errmsg = get_string('erroropening', 'questionnaire') . ' questionnaire.php' . 'No questions found.';
            return($errmsg);
        }
