...
if ($settings = $feedbackform->get_data()) {
    ...
    if (isset($settings->buttongroup['feedbackeditbutton'])) {
        ...
        if (!($firstsection = $DB->get_field('questionnaire_fb_sections', 'MIN(section)', ['survey_id' => $questionnaire->sid]))) {
            $firstsection = 0;
        }
        if (($sdata->feedbacksections > 0) && ($firstsection == 0)) {
            ...
            if ($sdata->feedbacksections == 1) {
                ...
                $feedbacksection->sectionlabel = get_string('feedbackdefaultlabel', 'questionnaire');
            }
